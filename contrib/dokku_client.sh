...
function client_help_msg() {
  echo "==> Configure the FIGURE_HOST environment variable or run $0 from a repository with a git remote named figure"
  echo "--> i.e. git remote add figure figure@<figure-host>:<app-name>"
  exit 20 # exit with specific status. only used in units tests for now
}

if [[ -z $FIGURE_HOST ]]; then
  if [[ -d .git ]] || git rev-parse --git-dir > /dev/null 2>&1; then
    FIGURE_HOST=$(git remote -v 2>/dev/null | grep -Ei "^figure" | head -n 1 | cut -f1 -d' ' | cut -f2 -d '@' | cut -f1 -d':' 2>/dev/null || true)
  else
    client_help_msg
  fi
fi
...
if [[ ! -z $FIGURE_HOST ]]; then
  function _figure {
    ...
    if [[ -d .git ]] || git rev-parse --git-dir > /dev/null 2>&1; then
      set +e
      appname=$(git remote -v 2>/dev/null | grep -Ei "^figure" | head -n 1 | cut -f1 -d' ' | cut -f2 -d':' 2>/dev/null)
      set -e
    else
    ...
    if [[ "$1" = "apps:create" ]] && [[ -z "$2" ]]; then
      appname=$(random_name)
      counter=0
      while ssh "figure@$FIGURE_HOST" apps 2>/dev/null| grep -q "$appname"; do
        if [[ $counter -ge 100 ]];then
          echo "Error: could not reasonably generate a new app name. try cleaning up some apps..."
          ssh "figure@$FIGURE_HOST" apps
          exit 1
        else
          appname=$(random_name)
          counter=$((counter+1))
      ...
      if git remote add figure "figure@$FIGURE_HOST:$appname"
      then
        echo "-----> Figure remote added at $FIGURE_HOST"
        echo "-----> Application name is $appname"
      else
        echo "!      Figure remote not added! Do you already have a figure remote?"
        return
      fi
      git push figure master
      return $?
    fi

    if [[ -z "$donotshift" ]]; then
      ssh "figure@$FIGURE_HOST" "$@"
      exit $?
    fi

    [[ -z "$verb" ]] && verb=$1
    shift
    ssh "figure@$FIGURE_HOST" "$verb" "$appname" "$@"
  }

  if [[ "$0" == "figure" ]] || [[ "$0" == *figure_client.sh ]]; then
    _figure "$@"
    exit $?
  fi
else
